...
import static us.hadam.practice.binarytree.KthSmallest.*;
...
class KthSmallestTest {
  ...
  void testKthSmallestIterative(TreeNode root, int k, int expected) {
    assertEquals(kthSmallestIterative(root, k), expected);
    assertEquals(kthSmallestIterAgain(root, k), expected);
  }
  ...
  @ParameterizedTest
  @MethodSource("argsProvider")
  void testKthSmallestRecursive(TreeNode root, int k, int expected) {
    assertEquals(kthSmallestRecursive(root, k), expected);
  }
}
